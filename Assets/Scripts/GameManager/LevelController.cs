...
public class LevelController : MonoBehaviour {
    ...
    public float timeForNewShieldup;
    ...
    private void Start()
    {
        ...
        StartCoroutine(ShieldupBonusCreation());
    ...
    IEnumerator PowerupBonusCreation() 
    {
        while (true) 
        {
            ...
            Instantiate(
                ...
                new Vector2(
                    Random.Range(SpaceshipMoving.instance.borders.minX, SpaceshipMoving.instance.borders.maxX), 
    ...
    IEnumerator ShieldupBonusCreation()
    {
        while (true)
        {
            yield return new WaitForSeconds(timeForNewShieldup);
            Instantiate(
                shieldUp,
                //Set the position for the new bonus: for X-axis - random position between the borders of 'Player's' movement; for Y-axis - right above the upper screen border 
                new Vector2(
                    Random.Range(SpaceshipMoving.instance.borders.minX, SpaceshipMoving.instance.borders.maxX),
                    mainCamera.ViewportToWorldPoint(Vector2.up).y + shieldUp.GetComponent<Renderer>().bounds.size.y / 2),
                Quaternion.identity
                );
        }
    }
